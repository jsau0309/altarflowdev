// Document AI implementation using gcloud auth for local development
// This approach works when you can't create service account JSON keys

import { exec } from 'child_process';
import { promisify } from 'util';

const execAsync = promisify(exec);

export interface ExtractedReceiptData {
  vendor: string | null;
  total: number | null;
  date: string | null;
  netAmount?: number | null;
  taxAmount?: number | null;
  invoiceNumber?: string | null;
  currency?: string | null;
  items?: Array<{
    description: string;
    amount: number;
    quantity?: number;
  }>;
}

// Get access token using gcloud CLI
async function getAccessTokenFromGcloud(): Promise<string> {
  try {
    const { stdout } = await execAsync('gcloud auth application-default print-access-token');
    return stdout.trim();
  } catch (error) {
    console.error('Error getting gcloud access token:', error);
    throw new Error('Failed to get access token from gcloud. Make sure gcloud CLI is installed and authenticated.');
  }
}

// Alternative: Use environment variable if provided (for production with workload identity)
async function getAccessToken(): Promise<string> {
  // Option 1: Check if we have a direct access token in env (for CI/CD)
  if (process.env.GOOGLE_ACCESS_TOKEN) {
    // Using access token from environment variable
    return process.env.GOOGLE_ACCESS_TOKEN;
  }

  // Option 2: Try to get token from metadata service (for Google Cloud environments)
  if (process.env.K_SERVICE || process.env.GOOGLE_CLOUD_PROJECT) {
    try {
      const response = await fetch(
        'http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/default/token',
        {
          headers: {
            'Metadata-Flavor': 'Google',
          },
        }
      );
      
      if (response.ok) {
        const data = await response.json();
        // Using access token from metadata service
        return data.access_token;
      }
    } catch (error) {
      // Not in Google Cloud environment, continue to next option
    }
  }

  // Option 3: Use gcloud CLI for local development
  console.log('Using gcloud CLI for authentication');
  return getAccessTokenFromGcloud();
}

export async function processReceiptWithDocumentAI(
  fileBuffer: Buffer,
  mimeType: string
): Promise<ExtractedReceiptData> {
  const projectId = process.env.GOOGLE_CLOUD_PROJECT_ID;
  const location = process.env.GOOGLE_CLOUD_LOCATION || 'us';
  const processorId = process.env.GOOGLE_DOCUMENT_AI_PROCESSOR_ID;

  if (!projectId || !processorId) {
    console.error('Document AI config check:', {
      hasProjectId: !!projectId,
      hasProcessorId: !!processorId,
      projectId: projectId || 'MISSING',
      processorId: processorId || 'MISSING',
      location
    });
    throw new Error('Document AI configuration missing. Please check environment variables.');
  }

  try {
    // Get OAuth2 access token
    const accessToken = await getAccessToken();
    
    // Build the REST API endpoint
    const endpoint = `https://${location}-documentai.googleapis.com/v1/projects/${projectId}/locations/${location}/processors/${processorId}:process`;

    // Prepare the request
    const requestBody = {
      rawDocument: {
        content: fileBuffer.toString('base64'),
        mimeType: mimeType,
      },
    };

    // Make the API call with OAuth2 token
    console.log('Document AI Request:', {
      endpoint,
      mimeType,
      contentLength: fileBuffer.length,
      authMethod: process.env.GOOGLE_ACCESS_TOKEN ? 'Environment Token' : 
                  (process.env.K_SERVICE || process.env.GOOGLE_CLOUD_PROJECT) ? 'Metadata Service' : 
                  'gcloud CLI'
    });

    const response = await fetch(endpoint, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${accessToken}`,
      },
      body: JSON.stringify(requestBody),
    });

    if (!response.ok) {
      const errorText = await response.text();
      console.error('Document AI API Error Response:', {
        status: response.status,
        statusText: response.statusText,
        errorBody: errorText
      });
      
      // Parse Google API error format
      try {
        const errorJson = JSON.parse(errorText);
        if (errorJson.error) {
          throw new Error(`Document AI Error: ${errorJson.error.message || errorJson.error.status || response.statusText}`);
        }
      } catch (e) {
        // If not JSON, use original text
      }
      
      throw new Error(`Document AI API failed: ${response.status} ${response.statusText} - ${errorText.substring(0, 200)}`);
    }

    const result = await response.json();
    
    if (!result.document) {
      throw new Error('No document returned from Document AI');
    }

    const { document } = result;
    
    // Extract entities from the document
    const entities = document.entities || [];
    
    // Log all entity types for debugging
    console.log('Available entity types:', entities.map((e: any) => ({
      type: e.type,
      mentionText: e.mentionText?.substring(0, 50),
      normalizedValue: e.normalizedValue
    })));
    
    // Helper function to find entity by type - prioritize exact matches
    const findEntity = (types: string[]): string | null => {
      // First pass: look for exact matches
      for (const type of types) {
        const entity = entities.find((e: any) => {
          const entityType = e.type?.toLowerCase() || '';
          const typeToMatch = type.toLowerCase();
          return entityType === typeToMatch;
        });
        if (entity) {
          return entity.mentionText || entity.normalizedValue?.text || null;
        }
      }
      
      // Second pass: look for partial matches if no exact match found
      for (const type of types) {
        const entity = entities.find((e: any) => {
          const entityType = e.type?.toLowerCase() || '';
          const typeToMatch = type.toLowerCase();
          // Don't match address fields when looking for supplier name
          if (typeToMatch.includes('name') && entityType.includes('address')) {
            return false;
          }
          return entityType.includes(typeToMatch) ||
                 entityType.replace(/_/g, ' ').includes(typeToMatch) ||
                 entityType.replace(/_/g, '').includes(typeToMatch.replace('_', ''));
        });
        if (entity) {
          return entity.mentionText || entity.normalizedValue?.text || null;
        }
      }
      return null;
    };

    // Helper function to extract monetary value
    const extractMoney = (types: string[]): number | null => {
      for (const type of types) {
        const entity = entities.find((e: any) => {
          const entityType = e.type?.toLowerCase() || '';
          const typeToMatch = type.toLowerCase();
          // Check exact match or contains
          return entityType === typeToMatch || 
                 entityType.includes(typeToMatch) ||
                 entityType.replace(/_/g, ' ').includes(typeToMatch) ||
                 entityType.replace(/_/g, '').includes(typeToMatch.replace('_', ''));
        });
        
        if (entity) {
          if (entity.normalizedValue?.moneyValue) {
            const money = entity.normalizedValue.moneyValue;
            // Convert to number (units + nanos/1e9)
            return Number(money.units || 0) + Number(money.nanos || 0) / 1e9;
          }
          
          // Fallback to text extraction
          const text = entity.mentionText || entity.normalizedValue?.text;
          if (text) {
            const cleaned = text.replace(/[^0-9.-]/g, '');
            const parsed = parseFloat(cleaned);
            return isNaN(parsed) ? null : parsed;
          }
        }
      }
      return null;
    };

    // Helper function to extract date
    const extractDate = (): string | null => {
      const dateEntity = entities.find((e: any) => 
        e.type?.toLowerCase().includes('date') ||
        e.type?.toLowerCase() === 'invoice_date' ||
        e.type?.toLowerCase() === 'due_date'
      );
      
      if (dateEntity?.normalizedValue?.dateValue) {
        const date = dateEntity.normalizedValue.dateValue;
        // Format as YYYY-MM-DD
        return `${date.year}-${String(date.month).padStart(2, '0')}-${String(date.day).padStart(2, '0')}`;
      }
      
      // Fallback to text
      return dateEntity?.mentionText || null;
    };

    // Extract line items if available
    const extractLineItems = () => {
      const items: ExtractedReceiptData['items'] = [];
      
      const lineItemEntities = entities.filter((e: any) => 
        e.type?.toLowerCase().includes('line_item') ||
        e.type?.toLowerCase().includes('item')
      );
      
      lineItemEntities.forEach((item: any) => {
        const properties = item.properties || [];
        
        const description = properties.find((p: any) => 
          p.type?.toLowerCase().includes('description') ||
          p.type?.toLowerCase().includes('item')
        )?.mentionText || item.mentionText || '';
        
        const amountProp = properties.find((p: any) => 
          p.type?.toLowerCase().includes('amount') ||
          p.type?.toLowerCase().includes('price')
        );
        
        let amount = 0;
        if (amountProp?.normalizedValue?.moneyValue) {
          const money = amountProp.normalizedValue.moneyValue;
          amount = Number(money.units || 0) + Number(money.nanos || 0) / 1e9;
        } else if (amountProp?.mentionText) {
          const cleaned = amountProp.mentionText.replace(/[^0-9.-]/g, '');
          amount = parseFloat(cleaned) || 0;
        }
        
        if (description && amount > 0) {
          items.push({ description, amount });
        }
      });
      
      return items.length > 0 ? items : undefined;
    };

    // Build the extracted data object - using exact field names from Document AI
    // Note: total_amount should be the final amount after tax
    const extractedData: ExtractedReceiptData = {
      vendor: findEntity(['supplier_name', 'supplier', 'vendor', 'merchant', 'vendor_name', 'merchant_name'])?.replace(/\n/g, ' ').trim() || null,
      total: extractMoney(['total_amount', 'total', 'grand_total', 'amount_due', 'total_price', 'final_total']),
      date: extractDate(),
      netAmount: extractMoney(['net_amount', 'subtotal', 'net_total', 'subtotal_amount']),
      taxAmount: extractMoney(['total_tax_amount', 'total_tax', 'tax_amount', 'tax']),
      invoiceNumber: findEntity(['invoice_id', 'invoice_number', 'receipt_number', 'document_number']) || null,
      currency: findEntity(['currency']) || 'USD',
      items: extractLineItems(),
    };
    
    // Additional logic: if we have net amount and tax but no total, calculate it
    if (!extractedData.total && extractedData.netAmount && extractedData.taxAmount) {
      extractedData.total = extractedData.netAmount + extractedData.taxAmount;
      console.log('Calculated total from net + tax:', extractedData.total);
    }
    
    // Fallback: if no total found, look for the largest money value (usually the total)
    if (!extractedData.total) {
      const allMoneyValues = entities
        .filter((e: any) => e.normalizedValue?.moneyValue)
        .map((e: any) => {
          const money = e.normalizedValue.moneyValue;
          return {
            type: e.type,
            value: Number(money.units || 0) + Number(money.nanos || 0) / 1e9
          };
        })
        .sort((a: any, b: any) => b.value - a.value);
      
      if (allMoneyValues.length > 0) {
        // The largest value is likely the total
        extractedData.total = allMoneyValues[0].value;
        console.log(`Using largest money value as total: $${extractedData.total} (type: ${allMoneyValues[0].type})`);
      }
    }
    
    // Log all money values found for debugging
    console.log('Money values extracted:', {
      total: extractedData.total,
      netAmount: extractedData.netAmount,
      taxAmount: extractedData.taxAmount,
      allMoneyEntities: entities
        .filter((e: any) => e.normalizedValue?.moneyValue || (e.type?.toLowerCase().includes('amount') || e.type?.toLowerCase().includes('total') || e.type?.toLowerCase().includes('tax')))
        .map((e: any) => ({
          type: e.type,
          value: e.normalizedValue?.moneyValue ? 
            (Number(e.normalizedValue.moneyValue.units || 0) + Number(e.normalizedValue.moneyValue.nanos || 0) / 1e9) :
            e.mentionText
        }))
    });

    // Log for debugging
    console.log('Document AI Extraction Results:', {
      entitiesFound: entities.length,
      extractedData,
    });

    return extractedData;

  } catch (error) {
    console.error('Document AI processing error:', error);
    throw new Error(`Failed to process document: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

// Helper function to map Document AI results to your expense format
export function mapToExpenseFormat(data: ExtractedReceiptData) {
  return {
    vendor: data.vendor || 'Unknown Vendor',
    amount: data.total || 0,
    date: data.date || new Date().toISOString().split('T')[0],
    taxAmount: data.taxAmount,
    invoiceNumber: data.invoiceNumber,
    currency: data.currency || 'USD',
    items: data.items,
  };
}